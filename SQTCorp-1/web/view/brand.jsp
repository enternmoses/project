        ...
        <div> <br> <br>
            ...
            <div>
                <h2 style="width: 100%; text-align: center; border-bottom: 2px solid #000; line-height: 0.1em; margin: 10px 0 20px;">
                    <span style="background:#fff; padding:0 10px; ">Đồng hồ OP</span>
                </h2>
            </div> <br>
                
            <div class="row" style="padding: 0px 100px;">
                <% ArrayList<Product> listOP = Database.getMiniProduct("Select * from product where brand ='OLYM PIANUS' or brand ='OGIVAL'");%>
                <% ArrayList<Product> listCitizen = Database.getMiniProduct("Select * from product where brand ='ORIENT' or brand ='SEIKO'");%>
                <% ArrayList<Product> listCouple = Database.getMiniProduct("Select * from product where sex='Unisex'");%>
                <%
                String pattern = "###,###.##";
                
                DecimalFormat decimalFormat = new DecimalFormat(pattern);
                
                for(int i=0; i<Math.min(8, listOP.size()); i++){ %>
                <div class="col-sm-3 product-item">
                    <div>
                        <img style="width: 100%;height: 50%;" src=<%= "../" + listOP.get(i).getImage() %> alt=""/>
                    </div>
                    <div style="padding-top: 20px;padding-bottom: 50px; width: 100%;height: 100%;text-align: center;">
                        <span><%= listOP.get(i).getName() %></span> <br> <br>
                                <span><strong><%=decimalFormat.format(listOP.get(i).getPrice())+ "đ"%></strong></span> <br>
                                
                                
                        <form action="edit" method="get">
                
                            <input type="hidden" name="action" value="add">
                            <input type="hidden" name="productCode" value=<%=listOP.get(i).getCode() %>>
            ...
            <div>
                <h2 style="width: 100%; text-align: center; border-bottom: 2px solid #000; line-height: 0.1em; margin: 10px 0 20px;">
                    <span style="background:#fff; padding:0 10px; ">Đồng hồ Citizen</span>
                </h2>
            </div> <br>
                
            <div class="row" style="padding: 0px 100px;">
                <%
                for(int i=0; i<Math.min(8, listCitizen.size()); i++){ %>
                <div class="col-sm-3 product-item">
                    <div>
                        <img style="width: 100%;height: 50%;" src=<%= "../" + listCitizen.get(i).getImage() %> alt=""/>
                    </div>
                    <div style="padding-top: 20px;padding-bottom: 50px; width: 100%;height: 100%;text-align: center;">
                        <span><%= listCitizen.get(i).getName() %></span> <br> <br>
                                <span><strong><%=decimalFormat.format(listCitizen.get(i).getPrice())+ "đ"%></strong></span> <br>
                                
                                
                        <form action="edit" method="get">
                
                            <input type="hidden" name="action" value="add">
                            <input type="hidden" name="productCode" value=<%=listCitizen.get(i).getCode() %>>
                            <button type="button" class="btn btn-danger" name="add" style="margin-top: 20px;">Add Product</button>
                        </form>
                    </div>
            ...
            <div>
                <h2 style="width: 100%; text-align: center; border-bottom: 2px solid #000; line-height: 0.1em; margin: 10px 0 20px;">
                    <span style="background:#fff; padding:0 10px; ">Đồng hồ đôi</span>
                </h2>
            </div> <br>
            ...
            <div class="row" style="padding: 0px 100px;">
                ...
                <%
                for(int i=0; i<Math.min(8, listCouple.size()); i++){ %>
                <div class="col-sm-3 product-item">
                    <div>
                        <img style="width: 100%;height: 50%;" src=<%= "../" + listCouple.get(i).getImage() %> alt=""/>
                    </div>
                    <div style="padding-top: 20px;padding-bottom: 50px; width: 100%;height: 100%;text-align: center;">
                        <span><%= listCouple.get(i).getName() %></span> <br> <br>
                                <span><strong><%=decimalFormat.format(listCouple.get(i).getPrice())+ "đ"%></strong></span> <br>
                                
                                
                        <form action="edit" method="get">
                
                            <input type="hidden" name="action" value="add">
                            <input type="hidden" name="productCode" value=<%=listCouple.get(i).getCode() %>>
                            <button type="button" class="btn btn-danger" name="add" style="margin-top: 20px;">Add Product</button>
                        </form>
                    </div>
